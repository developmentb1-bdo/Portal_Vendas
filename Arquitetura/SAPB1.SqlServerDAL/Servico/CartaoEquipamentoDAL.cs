...
namespace SAPB1.SqlServerDAL.Servico
{
    ...
    {
        public CartaoEquipamentoDAL() { }

        string tSQLBase = "SELECT insID, customer, custmrName, contactCod, directCsmr, drctCsmNam, manufSN, internalSN, warranty, wrrntyStrt, wrrntyEnd, responsVal, responsUnt, " +
            "itemCode, itemName, itemGroup, manufDate, delivery, deliveryNo, invoice, invoiceNum, dlvryDate, cntctPhone, street, block, zip, city, county, country, state, instLction, " +
            "contract, cntrctStrt, cntrctEnd, attachment, objType, logInstanc, userSign, createDate, userSign2, updateDate, Building, status, replcIns, repByIns, technician, territory, " +
            "AtcEntry, Transfered, AddrType, Instance, StreetNo " +
            "FROM OINS ";
